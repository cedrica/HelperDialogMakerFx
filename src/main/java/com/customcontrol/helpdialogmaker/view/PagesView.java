...
public class PagesView implements Initializable {
    ...
    public void setHtmlContent(String content) {
        ...
        menu += repeatProcess(Session.createInstance().pages, 0);
        menu += "</ul></div>";
        menu += content + "</body></html>";
        this.htmlContent = menu;
        webEngine.loadContent(menu);

    }

    private String repeatProcess(List<PageView> subPages, int wasOpenForChildren) {
        String apres = "";
        for (PageView pageView : subPages) {
            PageData pageData = pageView.getPageViewModel().getPageData();
            apres += "<li><h4><a href=\"#section"
                    + pageData.getIndex() + "\">" + pageData.getIndex() + " " + pageData.getName() + "</a></h4>";
            if (pageView.getSubPages().size() > 0) {
                apres += "<ul>";
                apres += repeatProcess(pageView.getSubPages(), ++wasOpenForChildren);
    ...
    private void recursivAddSubPage(TreeItem<String> root, TreeItem<String> childItem, PageData pageData,
                                    PageView pageView) {
